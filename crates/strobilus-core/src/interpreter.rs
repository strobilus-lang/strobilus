        ...
        match command.inner_kind() {
            CommandKind::AddParent(expr, expr1) => {
                ...
                match (child_value.value_kind(), parent_value.value_kind()) {
                    (
                        ValueKind::Lit(Literal::EntityUID(child_uid)),
                        ValueKind::Lit(Literal::EntityUID(parent_uid)),
                    ) => {
                        self.entity_store
                            .add_parent(child_uid, (**parent_uid).clone())?;
                        Ok(())
                    }
                    (ValueKind::Lit(Literal::EntityUID(_)), _) => {
            ...
            CommandKind::UpdateEntity(uid, attributes, ancestors, tags) => {
                let uid_value =
                    self.clone()
                        .evaluate::<()>(request.clone(), uid)
                        .and_then(|v| {
                            let kind = v.value_kind().clone();
                            if matches!(kind, ValueKind::Lit(Literal::EntityUID(_))) {
                                Ok(kind)
                            } else {
                                Err(Box::new(std::io::Error::new(
                                    std::io::ErrorKind::InvalidData,
                                    "UID must evaluate to an EntityUID",
                                )))
                            }
                        })?;

                let attributes_value = self
                    .clone()
                    .evaluate::<()>(request.clone(), attributes)
                    .and_then(|v| {
                        let kind = v.value_kind().clone();
                        if matches!(kind, ValueKind::Record(_)) {
                            Ok(kind)
                        } else {
                            Err(Box::new(std::io::Error::new(
                                std::io::ErrorKind::InvalidData,
                ...
                let ancestors_value = self
                    .clone()
                    .evaluate::<()>(request.clone(), ancestors)
                    .and_then(|v| {
                        let kind = v.value_kind().clone();
                        if matches!(kind, ValueKind::Set(_)) {
                            Ok(kind)
                        } else {
                            Err(Box::new(std::io::Error::new(
                                std::io::ErrorKind::InvalidData,
                ...
                match (uid_value, attributes_value, ancestors_value, tags_value) {
                    (
                        ...
                        // Convert Arc<BTreeMap<SmolStr, Value>> to HashMap<SmolStr, Value>
                        let attrs_map: HashMap<_, _> = (*attrs).clone().into_iter().collect();

                        // Convert Arc<HashSet<Literal>> to HashSet<EntityUID>
                        let ancestors_set: HashSet<EntityUID> = ancestors
                            .fast
                            .as_ref()
                            .expect("Ancestors must be a set")
                            .iter()
                            .filter_map(|lit| {
                                if let cedar_policy_core::ast::Literal::EntityUID(uid) = lit {
                                    Some((**uid).clone())
                                } else {
                                    None
                                }
                        ...
                        // Convert Arc<BTreeMap<SmolStr, Value>> to HashMap<SmolStr, PartialValue>
                        let tags_map: HashMap<_, _> = (*tags)
                            .clone()
                            .into_iter()
                            .map(|(k, v)| (k, v.into()))
                            .collect();

                        self.entity_store.update_entity(
                            (*uid).clone(),
                            attrs_map,
                            ancestors_set,
                            tags_map,
            ...
            CommandKind::RemoveEntity(uid) => {
                let uid_value = self.clone().evaluate::<()>(request.clone(), uid)?;

                if let ValueKind::Lit(Literal::EntityUID(uid)) = uid_value.value_kind() {
                    self.entity_store.remove_entity(uid)?;
                    Ok(())
                } else {
                    Err(Box::new(std::io::Error::new(
                        std::io::ErrorKind::InvalidData,
                        "Argument of removeEntity must be an EntityUID",
                    )))
                }
            }
            ...
            CommandKind::RemoveAttribute(expr, attribute) => {
                ...
                if let ValueKind::Lit(Literal::EntityUID(uid)) = value.value_kind() {
                    self.entity_store.remove_attribute(uid, attribute)?;
                    Ok(())
                } else {
                    Err(Box::new(std::io::Error::new(
                        std::io::ErrorKind::InvalidData,
