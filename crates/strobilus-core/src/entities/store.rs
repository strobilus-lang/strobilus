...
// ---------------------------------- READ SET ENTITY STORE IMPLEMENTATION ---------------------------------- 

use std::str::FromStr;

#[derive(Debug, Default, Clone)]
pub struct ReadSetEntityStore {
    inner: Entities,
    read_set: HashSet<EntityUID>,
}

impl ReadSetEntityStore {
    pub fn new(entities: Entities) -> Self {
        Self { 
            inner: entities, 
    ...
    pub fn reset_read_set(&mut self) {
        self.read_set = HashSet::new();
    }

    // TODO: IMPORTANT, CHANGED THIS FROM &self TO &mut self
    // COULD THIS BE A SECURITY ISSUE ?
    fn entity(&mut self, uid: &EntityUID) -> Option<Entity> {

        // Skip special Entities Justification::Permits and Justification Forbids
        let skip_vec = vec![
            EntityUID::from_str("Justification::\"Forbids\"").unwrap(),
            EntityUID::from_str("Justification::\"Permits\"").unwrap(),
        ];
        
        if !skip_vec.contains(uid) {
            self.read_set.insert(uid.clone());
        }
...
impl EntityStore for ReadSetEntityStore {
    fn add_parent(&mut self, uid: &EntityUID, parent: EntityUID) -> Result<(), EntityStoreError> {
        if let Some(mut entity) = self.entity(uid) {
            entity.add_parent(parent);
            self.update_entity(entity)?;
